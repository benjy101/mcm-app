...
export default function App() {
  ...
  const handleSubmit = async () => {
    // MAKE SURE ALL COMMENTS ARE FILLED
    ...
    try {
      const response = await fetch('https://api.clerk.com', {
        method: 'POST',
        headers: {
      ...
      Alert.alert('Error', 'An error occurred while submitting data. Please check your connection.');
    } finally {
      ...
      <ScrollView style={styles.scrollView}>
        ...
        <View style={styles.titleContainer}>
          <Text style={styles.title}>
            {'MOPANI COPPER MINES\nINFORMATION TECHNOLOGY\nFM-IT-046C\nNKANA DAILY NETWORK SYSTEM CHECKLIST'}
          </Text>
        </View>
        <TextInput
